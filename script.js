...
// LÓGICA DIRECTA: Salta únicamente al 3er día posterior a hoy de forma automática
...
// Sección 5: Cortes de Migración (Sincronización Total)
...
document.getElementById("btnProcesar").addEventListener("click", function() {
    ...
    // Captura limpia: Evita que falle si dejas campos opcionales vacíos
    ...
    // FORMATEO BLINDADO: Convierte de forma segura de AAAA-MM-DD a DD/MM/AAAA para tu Google Sheets
...
document.querySelectorAll(".check-familiar-res:checked").forEach(cb => seleccionadosRes.push(cb.value));let parientesRes = seleccionadosRes.length > 0 ? seleccionadosRes.join(", ") : "familiares directos";let subDetallesRes = "";if (document.getElementById("fam_res_hijos").checked) {let edadH = document.getElementById("hijos_res_edad").value || "no especificada";let milH = document.getElementById("hijos_res_ejercito").checked ? "sí cuentan con historial militar" : "no tienen historial militar";let tieneMed = document.getElementById("med_res_si").checked;let medH = tieneMed ? , confirmando un diagnóstico de: ${document.getElementById("diagnostico_res").value || "no detallado"} : ", sin complicaciones médicas ni espectro autista";subDetallesRes =  En el caso de los hijos residentes, tienen ${edadH} de edad, ${milH}${medH}.;}notaFamiliaRes = Tiene familiares con residencia legal permanente, incluyendo: ${parientesRes}.${subDetallesRes};} else {let comentarioRes = document.getElementById("comentarios_fam_res").value.trim();if (comentarioRes) notaFamiliaRes = No tiene familiares residentes directos, pero comenta lo siguiente: ${comentarioRes}.;}let notaCortes = "Actualmente no tiene próximas cortes de migración programadas.";if (document.getElementById("cortes_si").checked) {let fechaCorteProxima = document.getElementById("fecha_corte_proxima").value || "sin fecha";let comentariosCorteSi = document.getElementById("comentarios_corte_si").value.trim() || "sin comentarios adicionales";notaCortes = El cliente presenta una corte de migración próxima programada para el ${fechaCorteProxima} (${comentariosCorteSi}).;} else {let haPerdidoCorte = document.getElementById("perdido_si").checked;let haRevisadoDeportacion = document.getElementById("rev_dep_si").checked;if (haPerdidoCorte || haRevisadoDeportacion) {let detallePerdido = haPerdidoCorte ? Admite haber perdido una corte anteriormente debido a: ${document.getElementById("comentarios_perdido_nota").value || "razón no especificada"}.  : "No reporta cortes perdidas en el pasado. ";let detalleOrden = haRevisadoDeportacion ? Al verificar su estatus, confirma una orden de deportación registrada bajo el concepto: ${document.getElementById("fecha_orden_corte_input").value.trim() || "sin detalles registrados"}. : "No tiene conocimiento de que se le haya abierto una orden de deportación.";notaCortes = No tiene próximas cortes. ${detallePerdido}${detalleOrden};} else {notaCortes = "No tiene próximas cortes programadas ni historial de cortes perdidas o estatus de orden de deportación bajo este concepto.";}}// --- CONSTRUCCIÓN FINAL DE TEXTOS AUTOMÁTICOS ---// CORRECCIÓN FINAL: Extrae el primer nombre sin romper el SMS
let partesNombre = nombreCompleto.split(" ");
let primerNombre = partesNombre[0] ? partesNombre[0] : "Cliente";
et direccionExacta = direccionesOficinas[oficinaSeleccionada] || "Dirección no encontrada";let tagCita = ${horaFormateada}/IMMI/${nombreCompleto.toUpperCase()}/${telefono}/FERNANDO PADILLA;let mensajeSms = SomosDienerLaw Hola ${primerNombre} soy F. Padilla confirmando tu cita del ${fechaCitaFormateada} a las ${horaFormateada} en oficina ${direccionExacta} Manda STOP para darte de baja;let historialCompleto = `HISTORIAL DE CONVERSACIÓN DE LA LLAMADACLIENTE: ${nombreCompleto.toUpperCase()}TELÉFONO: ${telefono}FECHA DE NACIMIENTO: ${fechaNacimiento || "No registrada"}ESTADO CIVIL: ${estadoCivil.toUpperCase() || "No registrado"}FECHA DE ENTRADA A EE.UU: ${fechaEntrada || "No registrada"}DETALLES DEL DIAGNÓSTICO MIGRATORIO:${notaPeticion}${notaDetenciones}${notaFamiliaCiu}${notaFamiliaRes}${notaCortes}JUSTIFICACIÓN DE AGENDAMIENTO: ${justificacionFecha || "Cita programada dentro del rango estándar."}Confirmación de Cita: Sr/a ${nombreCompleto.toUpperCase()}, le esperamos en la oficina de ${oficinaSeleccionada} el ${fechaCitaFormateada} a las ${horaFormateada}.`;let filaExcelFormateada = ${fechaLlamadaAutomatica}\t${fechaCitaFormateada}\t${horaFormateada}\tNUEVO\t\t${telefono}\t${oficinaSeleccionada}\tFERNANDO PADILLA\t${ticketId}\t${zohoUrl};// INYECCIÓN INSTANTÁNEA EN CASILLASdocument.getElementById("resultadoTag").innerText = tagCita;document.getElementById("resultadoSms").innerText = mensajeSms;document.getElementById("resultadoHistorial").value = historialCompleto;document.getElementById("resultadoFilaExcel").value = filaExcelFormateada;// CONTROL DEL CONTADOR EN TIEMPO REALlet totalCaracteres = mensajeSms.length;let contenedorContador = document.getElementById("contadorCaracteres");contenedorContador.innerText = totalCaracteres;if (totalCaracteres > 160) {contenedorContador.style.color = "#e53e3e"; // Rojo} else {contenedorContador.style.color = "#2f855a"; // Verde}setTimeout(function() {boton.disabled = false;boton.innerHTML = "Procesar Mensajes Automáticos";}, 50);});// === 3. MOTOR UNIVERSAL DE COPIADO ===function activarBotonCopiado(idBotón, idElementoTexto, idSpanAlerta) {document.getElementById(idBotón).addEventListener("click", function() {let element = document.getElementById(idElementoTexto);let textoACopiar = element.value || element.innerText;if (!textoACopiar) return;navigator.clipboard.writeText(textoACopiar).then(() => {let alerta = document.getElementById(idSpanAlerta);alerta.innerText = "¡✅ Copiado exitosamente al portapapeles!";alerta.style.color = "#2f855a";setTimeout(function() { alerta.innerText = ""; }, 2000);});});}activarBotonCopiado("btnCopiarTag", "resultadoTag", "alertaTag");activarBotonCopiado("btnCopiarSms", "resultadoSms", "alertaSms");activarBotonCopiado("btnCopiarExcel", "resultadoFilaExcel", "alertaExcel");activarBotonCopiado("btnCopiarHistorial", "resultadoHistorial", "alertaHistorial");// === 4. BOTÓN LIMPIAR TODO (RESET COMPLETO DE CORTINAS) ===document.getElementById("btnLimpiar").addEventListener("click", function() {document.getElementById("formularioMigracion").reset();// Ocultar absolutamente todas las secciones condicionales al reseteardocument.getElementById("bloque_justificacion_fecha").style.display = "none";document.getElementById("bloque_peticion_extra").style.display = "none";document.getElementById("bloque_detalle_evidencia").style.display = "none";document.getElementById("bloque_detenciones_extra").style.display = "none";document.getElementById("bloque_fecha_deportacion").style.display = "none";document.getElementById("bloque_fam_ciu_si").style.display = "none";document.getElementById("bloque_hijos_ciu_detalles").style.display = "none";document.getElementById("bloque_diag_ciu").style.display = "none";document.getElementById("bloque_fam_ciu_no_comentarios").style.display = "none";document.getElementById("bloque_fam_res_si").style.display = "none";document.getElementById("bloque_hijos_res_detalles").style.display = "none";document.getElementById("bloque_diag_res").style.display = "none";document.getElementById("bloque_fam_res_no_comentarios").style.display = "none";document.getElementById("bloque_cortes_si").style.display = "none";document.getElementById("bloque_cortes_no").style.display = "none";document.getElementById("bloque_fecha_perdido").style.display = "none";document.getElementById("bloque_fecha_orden_corte").style.display = "none";// Limpiar textos de resultadosdocument.getElementById("resultadoTag").innerText = "";document.getElementById("resultadoSms").innerText = "";document.getElementById("resultadoHistorial").value = "";document.getElementById("resultadoFilaExcel").value = "";document.getElementById("contadorCaracteres").innerText = "0";document.getElementById("contadorCaracteres").style.color = "#4a5568";alert("Formulario limpio y apartados ocultados con éxito.");});
